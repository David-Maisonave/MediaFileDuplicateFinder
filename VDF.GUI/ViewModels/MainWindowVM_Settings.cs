...
#pragma warning restore CA1822 // Mark members as static
		...
		public IEnumerable<SettingsFile.SaveResultsOnExitOptions> SaveResultsOnExitOptions => Enum.GetValues<SettingsFile.SaveResultsOnExitOptions>();
		...
		public ReactiveCommand<Unit, Unit> AddIncludesToListCommand => ReactiveCommand.CreateFromTask(async () => {
			var result = await Utils.PickerDialogUtils.OpenDialogPicker(
				new FolderPickerOpenOptions() {
					AllowMultiple = true,
					Title = "Select folder to include in scan"
				}
				);
		...
		public ReactiveCommand<Unit, Unit> AddBlacklistToListCommand => ReactiveCommand.CreateFromTask(async () => {
			var result = await Utils.PickerDialogUtils.OpenDialogPicker(
				new FolderPickerOpenOptions() {
					AllowMultiple = true,
					Title = "Select folder to exclude from scan"
				});
		...
		public ReactiveCommand<Unit, Unit> StartScanCommandFromSettings => ReactiveCommand.CreateFromTask(async () => await StartScanCommandMain("FullScan"));
