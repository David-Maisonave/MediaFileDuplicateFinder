...
namespace VDF.GUI.ViewModels {
	public partial class MainWindowVM : ReactiveObject {
		...
		public enum CheckItemsFlags {
			...
			OldestDate = 32,
			NewestDate = 64,
			LowestDurationAndQuality = LowestDuration + LowestDuration,
			LowestDurationQualityAndShortestFileName = LowestDuration + LowestDuration + ShortestFileName
		}
		public void CheckItemsCommand(CheckItemsFlags checkitemsflags) {
			HashSet<Guid> blackListGroupID = new();
			foreach (var first in Duplicates) {
				if (blackListGroupID.Contains(first.ItemInfo.GroupId)) continue; //Dup has been handled already
				...
				//Duration first
				if ((checkitemsflags & CheckItemsFlags.LowestDuration) > 0 && !keep.ItemInfo.IsImage)
					keep = dupMods.OrderByDescending(d => d.ItemInfo.Duration).First();

				if ((checkitemsflags & CheckItemsFlags.LowestQuality) > 0) {
				...
				if ((checkitemsflags & CheckItemsFlags.ShortestFileName) > 0) {
					keep = dupMods.OrderByDescending(d => d.ItemInfo.FileNameLength).First();
				}
		...
		public ReactiveCommand<Unit, Unit> CheckLowestDurationQuality_And_ShortestNameCommand => ReactiveCommand.Create(() => CheckItemsCommand(CheckItemsFlags.LowestDurationQualityAndShortestFileName));
		public ReactiveCommand<Unit, Unit> CheckLowestDurationQualityCommand => ReactiveCommand.Create(() => CheckItemsCommand(CheckItemsFlags.LowestDurationAndQuality));
...
#pragma warning disable CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
				Dispatcher.UIThread.InvokeAsync(() => DeleteInternal(true));
			else if (dlgResult == MessageBoxButtons.No)
				Dispatcher.UIThread.InvokeAsync(() => DeleteInternal(false));
#pragma warning restore CS4014 // Because this call is not awaited, execution of the current method continues before the call is completed
		...
		public ReactiveCommand<Unit, Unit> DeleteSelectionPermanentlyCommand => ReactiveCommand.Create(() => {
			Dispatcher.UIThread.InvokeAsync(() => DeleteInternal(true, permanently: true));
		});
		...
		public ReactiveCommand<Unit, Unit> RemoveSelectionFromListAndBlacklistCommand => ReactiveCommand.Create(() => {
			Dispatcher.UIThread.InvokeAsync(() => DeleteInternal(false, blackList: true));
		});
		public ReactiveCommand<Unit, Unit> CreateSymbolLinksForSelectedItemsCommand => ReactiveCommand.Create(() => {
			Dispatcher.UIThread.InvokeAsync(() => DeleteInternal(false, blackList: false, createSymbolLinksInstead: true));
		});
		public ReactiveCommand<Unit, Unit> CreateSymbolLinksForSelectedItemsAndBlacklistCommand => ReactiveCommand.Create(() => {
			Dispatcher.UIThread.InvokeAsync(() => DeleteInternal(false, blackList: true, createSymbolLinksInstead: true));
		});
