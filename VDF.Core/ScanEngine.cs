...
#pragma warning restore CS8601 // Possible null reference assignment
			...
			try {
				Parallel.For(0, ScanList.Count, new ParallelOptions { CancellationToken = cancelationTokenSource.Token, MaxDegreeOfParallelism = Settings.MaxDegreeOfParallelism }, i => {
					...
					for (int n = i + 1; n < ScanList.Count; n++) {
						...
						if (isDuplicate &&
							entry.mediaInfo != null && compItem.mediaInfo != null &&
		...
		public async void CleanupDatabase() {
			await Task.Run(() => DatabaseUtils.CleanupDatabase());
		...
		void HighlightBestMatches() {
			...
			foreach (DuplicateItem item in Duplicates) {
				...
				//FileNameLength
				groupItems = groupItems.OrderBy(d => d.FileNameLength);
				bestMatch = groupItems.First();
				bestMatch.IsLongestFileNameLength = true;
				foreach (DuplicateItem otherItem in groupItems.Skip(1)) {
					if (otherItem.FileNameLength > bestMatch.FileNameLength)
						break;
					otherItem.IsLongestFileNameLength = true;
				}
				//DirNameLength
				groupItems = groupItems.OrderBy(d => d.DirNameLength);
				bestMatch = groupItems.First();
				bestMatch.IsLongestDirNameLength = true;
				foreach (DuplicateItem otherItem in groupItems.Skip(1)) {
					if (otherItem.DirNameLength > bestMatch.DirNameLength)
						break;
					otherItem.IsLongestDirNameLength = true;
				}
